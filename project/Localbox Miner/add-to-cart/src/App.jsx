...
const App = () => {
  ...
  function editFromCart(item, index) {
    setBoolean(true);
    setEditIndex(item); // Store the selected item for editing
    setEditIndex(index); // Store the index of the item being edited
  }
  function saveEdit() {
    const updatedCart = [...cart];
    updatedCart[editIndex] = editItem;
    setCart(updatedCart);
    setEditIndex(""); // Reset after saving
    setEditIndex("");
  }
  ...
  function handleCategory(e) {
    ...
    if (e.target.value === "all") {
      ...
      let filterCategory = allProducts.filter(
      ...
      <div className="flex text-center   mt-20">
        <div className=" h-20  p-4 bg-white shadow-lg rounded-xl items-center  text-center flex gap-6">
          ...
          <div className="flex flex-wrap gap-4">
            <button onClick={handleSort} className="button-low">
              Low to High
            </button>
            <button onClick={handleSort2} className="button-high">
              High to Low
            </button>
          </div>
        </div>

        <div className="flex ml-14 items-center gap-2 w-125 border h-15 border-gray-300 rounded-lg p-2 focus-within:ring-2 focus-within:ring-blue-400">
          <input
            type="text"
            placeholder="Search..."
            value={search}
        ...
        {editIndex === index ? (
          <div>
            <input
              ...
              onChange={(e) => setEditIndex({ ...editItem, title: e.target.value })}
          ...
          <>
            ...
            <button
              ...
              onClick={() => editFromCart(item, index)}
            >
              Edit
            </button>
            <button
